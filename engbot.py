...
# 4 сайта, которые сразу оценят уровень английского (все параметры + сразу скажут уровень)
...
def level(message):
    ...
    bt4 = types.InlineKeyboardButton(
        'LanguageLevel',
        url='https://www.languagelevel.com/english/'
    )

    markup.add(bt3, bt4)
    text = (
        "Знание иностранного языка часто определяют от уровня A1 до близкого к уровню носителей языка C2.\n"
...
# 4 сайта для анализа сильных сторон (чтение, разговор, грамматика)
...
def level(message):
    ...
    bt3 = types.InlineKeyboardButton(
        'EnglishGrammar.org',
        url='https://www.englishgrammar.org/quizzes/'
    )
    bt4 = types.InlineKeyboardButton(
        'Speechling  ',
        url='https://speechling.com/ru/'
    )

    markup.add(bt3, bt4)
    text = (
        "Знание своих сильных сторон в английском (чтение, разговор, грамматика) помогает эффективно"
        " планировать обучение и сосредоточиться на развитии слабых навыков. Это повышает мотивацию, так"
        " как видно прогресс и области, где уже есть успехи. Кроме того, понимание сильных сторон позволяет "
        "использовать их в реальных ситуациях для более уверенного общения и понимания языка.\n"
...
# 4 сайта для анализа слабых сторон (чтение, разговор, грамматика)
...
def level(message):
    ...
    bt3 = types.InlineKeyboardButton(
        'English-Test.net',
        url='https://www.english-test.net/'
    )
    bt4 = types.InlineKeyboardButton(
        'Dictation.io',
    ...
    markup.add(bt3, bt4)
    text = (
        "Знание своих слабых сторон в английском (чтение, разговор, грамматика) помогает определить, на"
        " что нужно обратить больше внимания при изучении. Это позволяет строить более эффективный план обучения "
        "и избегать повторения ошибок. Осознание слабых мест стимулирует постоянное развитие и улучшение навыков, "
        "что ведёт к уверенности в языке и более быстрому прогрессу.\n"
